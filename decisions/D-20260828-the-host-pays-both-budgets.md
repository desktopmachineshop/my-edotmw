...
`just bench-render … HOST=1` is the instrument.
...
- **D-088's "player-hosted first, official dedicated later" is now a
  ...
  work.
- **`just bench-render HOST=1` exists** and takes `PRESET` and `HULLS`.
  Every knob defaults to the client-only behaviour, so a bare
  `just bench-render` measures exactly what it always did.
