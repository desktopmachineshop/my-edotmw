...
- **The farm registry is DERIVED from the buildings, never accumulated.**
  ...
  buildings and is called from `server._buildings_changed()`, beside
  `_refresh_passability()`, because the two answer the identical
  question — "the set of living buildings changed". Stock already grown
  survives the resync; a razed farm's entry does not. Deriving rather
  than hooking completion is what stops the sandbox's instant build, the
  upgrade path and `Scenario.apply_player` each needing to remember
  (#119's lesson: the handover nothing performs is the dangerous half).
