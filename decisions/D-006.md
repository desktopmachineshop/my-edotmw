...
**Amended 2026-08-19 — the corrected revisit trigger has fired, on
purpose, and this is the explicit revision it demanded**
(D-20260819-tier-three-lives-on-the-render-side, under the owner's
acceptance of all three tiers in
D-20260818-rome-total-war-formations-in-three-tiers and
D-20260818-battle-quality-outranks-player-count). The scope of the three
clauses changes as follows:

- **Clause 1 (purity) is unchanged for everything authoritative.** The
  wire carries squads; `Formation` stays pure; every OUTCOME reads
  positions derived from replicated squad state. Nothing about the
  40x arithmetic moved.
- **Clause 2 (one-way cosmetics) now explicitly permits per-soldier
  INTEGRATION STATE on the render path** — persistent slot assignments,
  eased positions, relaxation offsets — under three conditions, each of
  which is a test rather than a promise:
  1. **One-way:** never read by simulation, never on any wire, never in
     any hash (the boundary scans extend to the new state);
  2. **Bounded:** a drawn man stays within `MAX_RENDER_DRIFT` (1.5
     world units) of his authoritative slot, so selection, culling and
     every screen-space read built on authoritative data stays valid;
  3. **Outcome-blind:** nothing outcome-affecting consumes a rendered
     position. Two clients may draw one melee differently by
     centimetres; they may never RESOLVE it differently.
- **Clause 3 (deterministic reassignment) is unchanged on the
  authoritative side** — `alive` is still the only formation input a
  death changes — while the render layer may now WALK survivors into
  the restamped slots instead of teleporting them.

**The NEW revisit trigger** (replacing the corrected one, which is now
spent): any mechanic that wants an OUTCOME to depend on a drawn
position — per-man hit tests, render-driven collision, anything reading
the render layer back — reopens the original choice (network ~40,000
entities, or accept divergence) as a NEW decision, never as a patch.
Likewise raising `MAX_RENDER_DRIFT` past selection's click tolerance
requires selection to read drawn positions first.
