...
## Amendment, 2026-08-29 — the handover is DEFERRED to M8, the identity is not

**Ruling on #356.** While this sat, #292/#318 landed on `main`: a
disconnecting player's army is wiped **and** their buildings razed,
because a quitter's undefended base kept them ACTIVE, `_check_victory`
never fired, and a rage-quit 1v1 ran to the time cap. It is guarded by
seven assertions in `tests/test_disconnect_elimination.gd`, including
*"the remaining player wins rather than running to the cap"*.

That and D-090's repossession **cannot both hold**: an AI-held seat is
still playing, so the remaining player does not win. It is exactly one
call site, and the collision is a design decision rather than a merge.

**#292/#318 wins today.** It is landed, guarded, and fixed an *observed*
stall; D-090 is M8's rule and this issue was always scoped as its
ENGINEERING half. So what ships is:

- **identity** — `PlayerIdentity`, the IDENTIFY opcode (40, graduated
  from reservation to allocation as #362's registry intends), seat
  binding, and reclaim;
- **not the policy** — `_on_disconnect` keeps #292/#318's wipe.

### The dormancy this creates, and why it is not silent

`hand_seat_to_ai` therefore ships **with no production caller**, which is
the declared-and-unread family this project keeps paying for — D-055's
`BuildingSim.damage()` sat that way for two milestones and no match could
be won. Three things make this different, and all three were required by
the ruling rather than offered:

1. it is **exercised end to end by test**
   (`test_the_deferred_handover_leaves_the_army_standing`), driven at the
   entry point M8 will wire rather than through the disconnect path;
2. the **site says so** — `_on_disconnect` carries a comment naming
   D-090, the deferral, and the function waiting for a caller;
3. a **guard checks that comment survives**
   (`test_the_deferred_wiring_is_named_where_it_will_land`), so the
   explanation cannot be deleted while the dead function remains. A
   future reader meets a documented pause, not an orphan to tidy away.

### What M8 owes

When D-090 is wired here, **#292/#318's seven guards get re-derived at
that point** — its own entry already predicts this, since a seat that
keeps playing makes "the remaining player wins" false by design. That is
the work, and it belongs with the change that causes it.
