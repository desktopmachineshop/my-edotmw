...
| Command | What it does |
|---|---|
| `just quick-test [SEED] [SANDBOX]` | **The fastest way to see a real match.** You + 3 AI opponents, everyone's civ (yours included) drawn randomly, no lobby to click through. Starts a detached server, waits for it to come up, launches the native GUI client connected to it, and tears everything down on exit. Needs native Godot (`just bootstrap` first). Re-run with the same `SEED` (default `1337`) to reproduce the same civ draw. `SANDBOX` (default `auto`) is D-077's dev build: on for an agent worktree, off on `main` — `just quick-test 1337 0` forces it off. |
...
| `just run-server [AI] [MAP] [LOBBY]` | Headless authoritative server, foreground. The first argument seats N computer opponents without a lobby (round-robin civs by default); the third, `1`, holds the server in the lobby instead of starting immediately — the world isn't generated until an admin presses Start, since its size/seed/shape are still being chosen there. **Arguments are positional**: `just run-server 3`, not `just run-server AI=3` — that binds the whole string "AI=3" to AI, and it only ever worked because `int()` strips the non-digits. `just run-server LOBBY=1` binds to AI the same way and gives you one AI and no lobby. Both fail loudly now (D-20260817-recipe-args-are-positional). |
