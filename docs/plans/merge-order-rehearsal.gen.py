...
w('comparisons green.\n\n')
w('**The merged tree fielded 11 squads where single-PR trees fielded 32-34, and\n')
w('that was a DEFECT, not the union playing differently.** An earlier version of\n')
w('this page guessed it was farms, techs, civ knobs and levy costs stacking; it\n')
w('was none of them. Cause found by #88 (#376): a load-test bot never sent a\n')
w('produce order AT ALL — not refused, never asked, which is why no server log\n')
w('ever showed a production refusal. `MatchState` seats a player with\n')
w('`civ = random` and only `_on_match_started` resolves it; under `--lobby=0` a\n')
w('bot connects AFTER the match begins, so its seat still reads `random`,\n')
w('`for_civ_archetype` answers null, and `BotBuildPlan._resolve` treated\n')
w('`random` as a real civ instead of falling back. Production was skipped\n')
w('silently for the whole match.\n\n')
w('**Re-measured on this rehearsal tree with the root fix (#380) merged in**,\n')
w('same host and method:\n\n')
w('| tree | squads | duration | squads/s | `nodes_felled` |\n|---|---|---|---|---|\n')
w('| single-PR baseline | 33 | 129.2 s | 0.255 | ~15-20 |\n')
w('| union (as rehearsed) | 11 | 129.2 s | **0.085** | 3 |\n')
w('| union + #380 | 22 | 142.3 s | **0.155** | 16 |\n\n')
w('Casualties 37 -> 69, 0 desyncs over 476 state-hash checks, 0 dropped ticks,\n')
w('0 ticks over budget. A residual gap to baseline remains and is NOT attributed\n')
w('here — do not read 0.155 as "fixed", only as "most of it was this".\n\n')
w('**The lesson worth carrying is about the instrument.** `build=` names the\n')
w('CHEAPEST thing a bot currently wants, so a bot with no economy reports the\n')
w('cheapest want it cannot afford. That made the farm look causal: remove farms\n')
w('and the same dead bot reports `cannot afford barracks` instead, which is\n')
w('exactly what a HEALTHY tree reports for an entirely different reason. **The\n')
w('label discriminates cost, not health** — I ran a five-run A/B off that\n')
w('reading, correctly excluded the balance cluster and #246, and still had the\n')
w('wrong candidate. `nodes_felled` collapsing to 3 was the same defect wearing a\n')
w('second face: with no crews there is nothing to fell.\n\n')
...
w('**END STATE, and it is what `origin` says.** #243 at tip **`741eb68`**\n')
w('("drop all four again — final arbitration"), all four grants dropped,\n')
w('**#324 merges BEFORE #243**, and the merge takes **zero conflicts and\n')
w('zero operator decisions** — with the four dropped, #243 makes no net\n')
w('change on those lines against `origin/main`, so there is nothing for a\n')
w('merger to resolve or to choose.\n\n')
w('Verified by performing it rather than describing it: `main` → #324 →\n')
w('#243 is clean at BOTH steps, and the final granted set is exactly\n')
w('`emberdeep_heavy=testudo`, `emberdeep_levy=shield_wall`,\n')
w('`gildedreach_spearmen=shield_wall`, with `test_fighting_styles` 6/6.\n\n')
w('Earlier drafts of this ruling predicted "one expected conflict on\n')
w('`emberdeep_heavy`, resolved #324\'s way". That was true of a superseded\n')
w('TWO-deletion form and is **not** true of what shipped. **Any future\n')
w('question about this is answered by diffing `origin`** — during the close,\n')
w('a stale local `main` and a stale ruling each nearly wrote a conflict into\n')
w('this record that cannot occur.\n\n')
