...
can play one. **That was written here as `run-server AI=3` for a
milestone and seated ZERO opponents** — just takes arguments
positionally, so the string "AI=3" bound to AI and `int()` read it as 0.
It fails loudly now (D-20260817-recipe-args-are-positional, #89), but
any session that typed it played alone.
