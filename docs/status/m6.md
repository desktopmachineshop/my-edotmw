...
here for walls, and it is still open: **allied AI behaviour is exercised
by unit tests and by nothing else.** A teamed ladder needs a server CLI
for teams *and* a fix to a dormant gap where `_sim.teams` is never handed
to the simulation on the no-lobby path (`--lobby=0` — the ladder,
`test-load` and `test-scenario` all run it), so allies seated there would
be allies the simulation never learns about. And `_enemy_target` was the
