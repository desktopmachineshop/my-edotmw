...
(D-054); `just run-server 3` seats opponents without a lobby so a human
can play one. **Written here as `run-server AI=3` for a milestone, which
worked only by accident** — just takes arguments positionally, so the
string "AI=3" bound to AI, and GDScript's `int()` strips non-digits and
returned 3. `run-server LOBBY=1` was not so lucky: it seats one AI and
no lobby. Both fail loudly now
(D-20260817-recipe-args-are-positional, #89), which also corrects D-075's
reading of the same trap.
...
**The 40.8 -> ~77 rise is ATTRIBUTED as of 2026-08-28**
(`D-20260828-the-m6-rise-has-a-name`, #304), and the answer is not the
one this paragraph assumed. Measured at a matched 120 squads on M4's own
8,192-cell map, **civs, teams and the economy cost nothing** — each lands
inside the instrument's own noise floor, and several passes measured them
*slower* switched off. The tick is **combat and separation**: 110.0 and
95.3 µs/squad of a 277.7 total, 74% between them, and the changes that
put them there are almost all LATER than M6 and each was priced when it
landed (`D-20260819-only-men-in-contact-fight` measured and accepted
combat 9.59 -> 33.19 at 24 squads on its own).

The map was the obvious suspect and is not the explanation either: the
shipped 32,592-cell map costs 328-399 against 8,192's 277, nearly all of
it flow-field expansion that #105 already owns.

**One thing was recovered outright**: separation was scanning a disk
sized for a clearance rule `D-20260821-a-fight-loosens-a-formation` had
already deleted, and narrowing it to the rule took that phase **95.3 ->
8.8 µs/squad** with a bit-for-bit identical answer. `just profile ladder`
is the instrument; read its `control` row before believing any small
difference in it.

*The original wording, kept because the reasoning is still worth having:*
The rise from M4's **40.8 µs/squad at 120 squads** to **~77** is *not*
the siege pass (measured with it disabled) and was for two milestones
unattributed to whichever of civs/teams/economy caused it. And worst-tick figures from that session are unreliable — a
...
**The AI had never heard of teams** (D-20260817-an-ai-knows-who-its-allies-are,
#83). `ai_player.gd` held zero references to alliance, so its targeting
read "not mine" as "hostile" and an allied AI marched its whole army onto
a **teammate's** town centre and milled there for the rest of the match:
`combat.gd` correctly refuses to let allies damage each other, so the
objective never cleared and the nearest-first scan re-picked it forever.
A livelock, and **the visible symptom is the correct rule meeting the
incorrect targeting**. Every scan in that file goes through one
`_hostile` predicate now (`not state.are_allied(who, player)`), the
metric built to diagnose the targeting included — it had counted an
ally's town as "an enemy base found".

Two things to carry forward. **The one configuration that breaks a thing
is very often the one nothing runs:** `just ai-ladder` is a free-for-all
and every AI fixture seated its AI on team 0, which is explicitly *not* a
team — so `are_allied` was false for every pair every automated exercise
of the AI has ever built. That is the same standing gap already noted
here for walls. And `_enemy_target` was the
first site, not the only one: the scouting fallback sent the army at a
teammate's spawn too, which is a wasted leg by construction because D-050
already shares their vision.

**That gap is closed as of 2026-08-18**
(D-20260818-allied-ai-is-exercised-by-something, #119). `just
test-ai-teams` plays real teamed all-AI matches — `--ai-teams=T` deals
the `--ai=N` seats across sides, and `_sim.teams` is now handed over on
**both** ways a match can start rather than only the lobby's. `just
ai-ladder` takes a `TEAMS` argument too, defaulting to 0 so every number
recorded before it stays comparable.

Three things worth knowing, none of them about the AI:

- **The dormant half was the dangerous half.** `--ai-teams` on its own
  would have seated allies the SIMULATION never heard of — every seat
  list and every client agreeing they are friends while they share no
  sight and shoot each other. Measured with the handover removed:
  `allies_seen` across four seats fell from ~12.0/10.0/13.7/9.3 to
  2.0/3.0/3.0/3.0, with nothing in the log saying why.
- **The harness was observed to catch #83 before it was trusted.** With
  the #83 fix reverted it reports **13 ally objectives across 2 of 4
  seats over 3 matches**; with it restored, 0 on the same seeds with the
  same `allies_seen`. One seed alone found only 1 of 4, which is why it
  plays several: the bug fires when an ALLY is the nearest known thing,
  and spawns are scattered (D-039), so that is a property of the seed.
- **`ally_objectives=0` is worth nothing on its own** — it is what a
  free-for-all, an unplayed match and an idle AI all report. The verdict
  fails first on every one of those.
