...
func stats_line() -> String:
	# BOTH investments' keys, on one line. `gate-check.sh` and the ladder
	# parse by NAME, so the union costs nothing and a seat that ran only
	# one of the two still reports the other's zeros — which is how a
	# harness tells "did not do it" from "was never asked".
	return "AI_STATS player=%d civ=%s profile=%s team=%d squads_peak=%d workers_peak=%d buildings=%d enemy_buildings_seen=%d allies_seen=%d ally_objectives=%d attacks=%d first_attack=%.1f first_attack_soldiers=%d peak_stockpile=%d peak_food=%d peak_wood=%d substituted=%d unreachable=%d scout_legs=%d afford_refusals=%d cap_refusals=%d defences_ordered=%d defences_standing=%d gate_orders=%d gates_sealed=%d buildings_lost=%d attacks_survived=%d defences_fought=%d wants_navy=%d docks=%d ships_peak=%d embarks=%d landings=%d naval_step=%s" % [
		...
		buildings_lost, attacks_survived, defences_fought,
