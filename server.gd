...
func _finish_build(peer, squad: int, def: BuildingDef, cell: Vector2i,
	...
	# terrain is (naval §4.1). Nearest-first through `disk_offsets`, which
	# D-067 sorted for exactly this walk-outward pattern, so two docks on
	# the same coast get the two different cells nearest each of them and
	# the choice is deterministic — a replay places them identically.
	if def.needs_shore:
...
func _return_to_lobby() -> void:
	...
	for peer in _local_clients:
		_local_clients[peer]["visible"] = {}
		_local_clients[peer]["known_buildings"] = {}
	for peer in _clients:
...
func _on_match_started() -> void:
	...
	# The host's own client needs the world's numbers exactly as much as
	# a remote one does — it derives the same terrain from them (D-049) —
	# and it is not in `_clients`, so it needs its own line rather than a
	# cast that would return null.
	for peer in _local_clients:
...
func _handle_chat(peer, data: PackedByteArray) -> void:
	...
	# `_recipients()`, not `_clients` — the THIRD time this exact drift has
	# been found, and that function's own doc records the first two
	# (`_replicate` merged in `_ai_clients`, `_broadcast_squad_info`
	# iterated `_clients` alone). It cost nothing while the only peers
	# outside `_clients` were AI seats, which do not read chat. #182 put a
	# HUMAN out there: a hosting player saw no chat at all, including
	# their own, and the cast below would have handed a null to `.send`
	# the moment anybody else spoke.
	var packet_targets := _recipients()
	for other in packet_targets:
