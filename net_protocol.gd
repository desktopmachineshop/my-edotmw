...
const OPCODES := {
	...
	# #225, the tech tree. Left OPCODES_RESERVED in this commit, per that
	# table's rule. In the techs RANGE, not the next free pair, so a
	# fourth tech message cannot collide with a fourth core one.
...
static func decode_squad_info(data: PackedByteArray) -> Array:
	...
	for i in range(count):
		...
		# #120, written between stance and cargo by the encoder. Read here
		# rather than inside the literal below: the cargo block is
		# variable-length, so a field read after it that was written
		# before it desynchronises the whole rest of the batch.
